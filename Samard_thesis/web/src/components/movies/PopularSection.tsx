...
import { getPopularMovies, getPopularTv } from "../../lib/apiClient";
...
export function PopularSection({ title, type }: PopularSectionProps) {
  const { user } = useAuth();
  const { entries } = useUserEntries();

  const { data, isLoading, error } = useQuery({
    queryKey: ["popular", type],
    queryFn: () => (type === "movies" ? getPopularMovies() : getPopularTv()),
    staleTime: 1000 * 60 * 60 * 24 // Cache for 24 hours (popular doesn't change as often)
  });
  ...
  if (error) {
    return <ErrorState message="Failed to load popular content." />;
  }
      ...
      <section className="mb-12">
        <div className="mb-6">
          ...
          <p className="text-gray-600 dark:text-gray-400">
            All-time popular {type === "movies" ? "movies" : "TV shows"}
          </p>
        </div>
  ...
  // Take first 12 items for popular (more than trending)
